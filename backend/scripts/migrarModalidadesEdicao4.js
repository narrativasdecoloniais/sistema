// Migra pro banco o conteúdo de "Submissão de trabalhos" que até aqui era
// hardcoded em frontend/lib/modalidadesSubmissao.js e
// frontend/lib/areasTematicasConversatorios.js, associando tudo à edição de
// número 4. Dados copiados literalmente (não importa os arquivos do
// frontend) pra este script continuar rodável mesmo depois que os dois
// arquivos hardcoded forem apagados.
//
// Rodar uma vez: node backend/scripts/migrarModalidadesEdicao4.js
// Idempotente: modalidades/áreas são upsert por slug; pessoas de cada área
// são recriadas do zero a cada execução (sem chave natural pra diffar).
const { PrismaClient } = require("@prisma/client");

const prisma = new PrismaClient();

const MODALIDADES_SUBMISSAO = [
  {
    slug: "conversatorios",
    nome: "Conversatórios",
...
const AREAS_TEMATICAS_CONVERSATORIOS = [
  {
    numero: 1,
    ...
    convidados: [{ nome: "Ramon Tupinambá", afiliacao: "Cacique do Povo Tupinambá" }],
    descricao: [
  ...
  },
  {
    ...
    coordenacao: ["Ivana Delfino (PPGCEN|Afeto|IdA-UnB)", "Adailson Costa (PPGCEN|Afeto|IdA-UnB)"],
    convidados: [{ nome: "Graça Veloso", afiliacao: "PPGCEN|Afeto|IdA-UnB" }],
    descricao: [
  ...
  },
  {
    ...
    convidados: [{ nome: "Kathia Núñez Patiño", afiliacao: "UNACH/REIR-México" }],
    descricao: [
  ...
  },
  {
    ...
    convidados: [{ nome: "Givânia Maria da Silva", afiliacao: "Coletivo de Educação da Conaq" }],
    descricao: [
  ...
  },
  {
    ...
    convidados: [{ nome: "Gina Vieira Pontes Albuquerque", afiliacao: "Gecria-UnB" }],
    descricao: [
  ...
  },
  {
    ...
    convidados: [{ nome: "Juami Aquino", afiliacao: "Comunidade Kalunga Vão do Moleque" }],
    descricao: [
    ...
    convidados: [
      {
        ...
        afiliacao: "Makota Kidoialê — Liderança de terreiro e do quilombo Manzo, Belo Horizonte/MG",
      },
      {
        nome: "Lucely Moraes Pio",
        afiliacao: "Raizeira, liderança da Articulação Pacari das Plantas Medicinais do Cerrado, Mineiros/GO",
      },
    ],
    descricao: [
  ...
  },
  {
    ...
    coordenacao: ["Paulo Brito do Prado (UFPI-Univasf)", "Marcela Amaral (UFG)"],
    convidados: [{ nome: "Flávia Valéria Cassimiro Braga Melo", afiliacao: "UEG" }],
    descricao: [
...
// A coordenação vem como "Nome (Instituição)" numa única string — mesma
// regex que dividirCoordenador() usava em frontend/lib/areasTematicasConversatorios.js.
function dividirCoordenador(pessoa) {
  const combinacao = pessoa.match(/^(.*)\s\((.+)\)$/);
  return combinacao ? { nome: combinacao[1], afiliacao: combinacao[2] } : { nome: pessoa, afiliacao: null };
}

async function main() {
  const edicao = await prisma.edicao.findUnique({ where: { numero: 4 } });
  if (!edicao) {
    throw new Error("Edição de número 4 não encontrada — abortando migração (nada foi gravado).");
  }

  for (const [indiceModalidade, modalidade] of MODALIDADES_SUBMISSAO.entries()) {
    const dadosModalidade = {
      nome: modalidade.nome,
      subtitulo: modalidade.subtitulo,
      prazoInicio: new Date(modalidade.prazoInicio),
      prazoFim: new Date(modalidade.prazoFim),
      resumoCurto: modalidade.resumoCurto,
      perguntaTitulo: modalidade.perguntaTitulo,
      descricao: modalidade.descricao.join("\n\n"),
      linkRotulo: modalidade.linkRotulo,
      rotuloItem: modalidade.rotuloItem,
      ordem: indiceModalidade,
    };

    const modalidadeDb = await prisma.modalidadeSubmissao.upsert({
      where: { edicaoId_slug: { edicaoId: edicao.id, slug: modalidade.slug } },
      create: { edicaoId: edicao.id, slug: modalidade.slug, ...dadosModalidade },
      update: dadosModalidade,
    });

    console.log(`Modalidade "${modalidade.nome}" ok (${modalidadeDb.id}).`);

    if (modalidade.slug !== "conversatorios") continue; // só Conversatórios tem áreas hoje

    for (const [indiceArea, area] of AREAS_TEMATICAS_CONVERSATORIOS.entries()) {
      const dadosArea = {
        titulo: area.titulo,
        descricao: area.descricao.join("\n\n"),
        ordem: indiceArea,
      };

      const areaDb = await prisma.areaSubmissao.upsert({
        where: { modalidadeSubmissaoId_slug: { modalidadeSubmissaoId: modalidadeDb.id, slug: area.slug } },
        create: { modalidadeSubmissaoId: modalidadeDb.id, slug: area.slug, ...dadosArea },
        update: dadosArea,
      });

      const pessoas = [
        ...area.coordenacao.map((str) => ({ ...dividirCoordenador(str), papel: "COORDENACAO" })),
        ...area.convidados.map((convidado) => ({ ...convidado, papel: "CONVIDADO" })),
      ];

      // Sem chave natural pra upsert individual de pessoa — apaga e recria
      // as pessoas da área a cada execução, seguro rodar o script de novo
      // (resultado final sempre igual).
      await prisma.pessoaAreaSubmissao.deleteMany({ where: { areaSubmissaoId: areaDb.id } });
      await prisma.pessoaAreaSubmissao.createMany({
        data: pessoas.map((pessoa, indice) => ({
          areaSubmissaoId: areaDb.id,
          nome: pessoa.nome,
      ...
      console.log(`  Área "${area.titulo.slice(0, 50)}..." ok — ${pessoas.length} pessoas.`);
    }
  }
