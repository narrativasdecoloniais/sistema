...
import styles from "./NavegacaoDias.module.scss";

export function idAbaDia(base, chave) {
  return `${base}-aba-${chave}`;
}

export function idPainelDia(base, chave) {
  return `${base}-painel-${chave}`;
}

export default function NavegacaoDias({ dias, chaveAtiva, onSelecionar, idBase }) {
      ...
      {dias.map((dia, index) => {
        ...
        return (
          <button
            ...
            type="button"
            role="tab"
            id={idAbaDia(idBase, dia.chave)}
            aria-controls={idPainelDia(idBase, dia.chave)}
            aria-selected={ativo}
            aria-label={completo}
            tabIndex={ativo ? 0 : -1}
