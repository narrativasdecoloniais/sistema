...
export default function FormularioSubmissao({
  ...
  useEffect(() => {
    ...
    async function carregar() {
      const dados = await listarModalidadesSubmissaoPublicas();
      if (cancelado) return;
      setModalidades(dados);

      const modalidadeInicial = dados.find((item) => item.slug === modalidadeSlugInicial) || dados[0];
      if (modalidadeInicial) {
        setModalidadeId(modalidadeInicial.id);
        const areaInicial = modalidadeInicial.areas.find((area) => area.slug === areaSlugInicial);
        if (areaInicial) setAreaId(areaInicial.id);
      }
    }
  ...
  async function aoSubmeter(evento) {
    ...
    try {
      ...
      notificar("Trabalho enviado com sucesso!", "sucesso");
      router.push(modalidadeSelecionada ? `/submissao/${modalidadeSelecionada.slug}` : "/");
    } catch (erro) {
