...
export default function TiposParticipacaoPainel({ tiposIniciais, podeEditar = true }) {
  ...
  async function excluirTipo(id) {
    ...
    try {
      await apiClient.delete(`/tipos-participacao/${id}`);
      setTipos((atual) => atual.filter((item) => item.id !== id));
      notificar("Tipo de participação excluído com sucesso.");
      router.refresh();
    } catch (erro) {
    ...
    <div className={styles.pagina}>
      ...
      {tipos.length === 0 ? (
        <div className={styles.vazio}>
          <p>Nenhum tipo de participação cadastrado ainda.</p>
          <p className={styles.vazioApoio}>
            Crie o primeiro tipo para poder cadastrar pessoas envolvidas nas atividades.
          </p>
        </div>
      ) : (
      ...
      {modalAberto && (
        <Modal
          titulo={tipoEmEdicao ? "Editar tipo de participação" : "Novo tipo de participação"}
          onFechar={fecharModal}
        >
          <TipoParticipacaoForm
            tipoParticipacaoInicial={tipoEmEdicao}
            aoSalvar={aoSalvar}
      ...
      {confirmandoId && (
        <ModalConfirmacao
          titulo="Excluir tipo de participação"
