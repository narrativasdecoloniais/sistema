...
export default function TiposAtividadePainel({ tiposIniciais }) {
  ...
  async function excluirTipo(id) {
    ...
    try {
      await apiClient.delete(`/tipos-atividade/${id}`);
      setTipos((atual) => atual.filter((item) => item.id !== id));
      notificar("Tipo de atividade excluído com sucesso.");
      router.refresh();
    } catch (erro) {
  ...
  return (
    <div className={styles.pagina}>
      <div className={styles.cabecalho}>
        <div>
          <h1 className={styles.titulo}>Tipos de atividade</h1>
          <p className={styles.descricao}>
            Catálogo de tipos (ex.: minicurso, oficina, mesa redonda) usado para classificar as
            atividades de todas as edições.
          </p>
        </div>
      ...
      {tipos.length === 0 ? (
        <div className={styles.vazio}>
          <p>Nenhum tipo de atividade cadastrado ainda.</p>
          <p className={styles.vazioApoio}>
            Crie o primeiro tipo para poder cadastrar atividades nas edições.
          </p>
        </div>
      ) : (
      ...
      {modalAberto && (
        <Modal titulo={tipoEmEdicao ? "Editar tipo de atividade" : "Novo tipo de atividade"} onFechar={fecharModal}>
          <TipoAtividadeForm
            tipoAtividadeInicial={tipoEmEdicao}
            aoSalvar={aoSalvar}
