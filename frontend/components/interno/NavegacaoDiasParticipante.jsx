...
import styles from "./NavegacaoDiasParticipante.module.scss";

export function idAbaDiaParticipante(base, chave) {
  return `${base}-aba-${chave}`;
}

export function idPainelDiaParticipante(base, chave) {
  return `${base}-painel-${chave}`;
}

export default function NavegacaoDiasParticipante({ dias, chaveAtiva, onSelecionar, idBase }) {
      ...
      {dias.map((dia, index) => {
        ...
        return (
          <button
            ...
            type="button"
            role="tab"
            id={idAbaDiaParticipante(idBase, dia.chave)}
            aria-controls={idPainelDiaParticipante(idBase, dia.chave)}
            aria-selected={ativo}
            aria-label={completo}
            tabIndex={ativo ? 0 : -1}
